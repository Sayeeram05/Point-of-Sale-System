...
class _OrderDetailDialogState extends State<OrderDetailDialog> {
  ...
  Future<void> handleMarkComplete() async {
    ...
    if (total != widget.order.totalPrice) {
      errorText.value =
    ...
    try {
      ...
      if (mounted) {
        if (widget.onMarkComplete != null) widget.onMarkComplete!();
  ...
  Future<void> _saveBill() async {
    ...
    try {
      final settingsJson = await ApiService.getB2BSettings();
      final settings = B2BSettings.fromJson(settingsJson);
      final pdfBytes = await BillPdfService.generateBillPdf(
        widget.order,
        settings,
      );
