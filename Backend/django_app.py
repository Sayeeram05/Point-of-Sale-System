...
- Starts Django's development server on 0.0.0.0:8000 in a background thread
...
def run_server():
    """Start Django's built-in server.  Called from a daemon thread."""
    try:
        logger.info("Starting Django server on 0.0.0.0:8000 ...")
        execute_from_command_line([
            'manage.py',
            'runserver',
            '0.0.0.0:8000',
            '--noreload',   # auto-reload is incompatible with frozen executables
        ])
    except Exception as exc:
...
def start_tray(stop_event: threading.Event):
    ...
    try:
        ...
        menu = Menu(
            MenuItem('Billing & Sales Server  |  port 8000', None, enabled=False),
            Menu.SEPARATOR,
        ...
        icon = pystray.Icon(
            name='BillingServer',
            icon=icon_image,
            title='Billing & Sales Server (port 8000)',
            menu=menu,
        )
