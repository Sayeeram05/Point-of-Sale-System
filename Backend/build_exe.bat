...
REM    - All packages installed via:  python -m pip install -r requirements.txt
REM    - No mysqlclient C extension is required because the frozen app uses PyMySQL
REM ============================================================
...
REM ---------------------------------------------------------------
REM  Step 2: Sync build dependencies from build-requirements.txt
REM ---------------------------------------------------------------
echo [2/7] Syncing build dependencies from build-requirements.txt ...
if not exist "build-requirements.txt" (
    echo [ERROR] build-requirements.txt not found.
    goto :error
)
python -m pip install --no-cache-dir -r build-requirements.txt --quiet
if %ERRORLEVEL% NEQ 0 (
    echo [ERROR] pip install failed. Check your internet connection or build-requirements.txt.
    goto :error
)
...
python -m PyInstaller django_app.spec --clean --noconfirm
if %ERRORLEVEL% NEQ 0 (
...
echo     5. A tray icon appears -- server is running on http://localhost:8001
